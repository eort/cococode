...
# to do fancy math shit
import numpy as np
# to search in system efficiently
import glob
# neat way of storing and writing data
import pandas as pd
# communication with the parallel port
import parallel
# doing some combinatorics
...
try:
    ...
    logging.error(f"{jsonfile} is not a valid config file.Stop it here.")
    sys.exit(-1)

"""
###          READ OUT JSON SIDECAR     ####
"""
# information on the timings of the sequence
timing_info =param['timing_info']
# information on all stimuli to be drawn
stim_info=param['stim_info']
# information on the psychopy window
win_info=param['win_info']
# information on response collection      
response_info=param['response_info']      
# Information on the reward schedule
reward_info=param['reward_info']
# information on files and variables for logging
logging_info=param['logging_info']
# information on all the MEG trigger values and names
trigger_info = param['trigger_info']
# make drawing the progressbar less of a pain
bar = stim_info['progress_bar']

"""
###         HANDLE INPUT DIALOG        ####
"""
# dialog with participants: Retrieve sub number, session number, and practice
input_dict = dict(sub_id=0, ses_id=param['ses_id'])
inputGUI =gui.DlgFromDict(input_dict, title='Experiment Info',
                          order=['sub_id', 'ses_id'], show=False)
while True:
    inputGUI.show()
    if inputGUI.OK == False:
        logging.warning("Experiment aborted by user")
        core.quit()
    if input_dict['ses_id'] in [f'{i:02d}'
                                for i in range(1, param['n_ses'] + 1)] +
                                ['scr','prac','intake']:
        break
    else:
        logging.warning('WARNING: {} is not a valid'
                        'ses_id'.format(input_dict['ses_id']))
...
logFileID = logging_info['skeleton_file'].format(input_dict['sub_id'],
            input_dict['ses_id'], param['name'], param['proj_id'],
...
# select colors from predefined options based on sub id and ses id, separately for screening and real sessions
